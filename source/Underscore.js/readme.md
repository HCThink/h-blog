...
[博客主页 : https://hcthink.github.io/HBook/](https://hcthink.github.io/HBook/)
...
```javascript
// underscop: [兼容 web 和 node, this 可以是 global: 参考: fe/sourceCode/js/eg/libPart/base/base.1.js]
(function () {

}.bind(this)());
// }.call(this);

// 或者: 目前大多 underscope 都是这种, 可以兼容 web 和 node 等多种宿主环境
(function () {
    const root = this;
}())
```
...
> 代码很简单.保留最初的 root._ 的引用. 当进入本库之后, root._ 被替换成当前库的引用.如果出现如下情况: 使用此方法
    ...
    __<引用多个不同版本的 underscore>__

    __<某个库也使用了 _ 作为简写方式挂载到宿主上.>__
...
2. 函数(多为 iteratee)包装器createCallback 1.8 之后被替换为optimizeCb, 实现没有太大改变
...
// 这个函数逻辑没有复杂的地方, 但是是函数式编程中的一个比较耐人寻味模式 currying
// currying : 参考
