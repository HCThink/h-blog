...
// 无传入方式的自调用块. 作用不多说, 避免污染全局.
// 1.5 的版本使用 .call(this) 的方式指定了 this.
// 对应的两个版本都没有. 实际上 un 在首行用 root 承接了 this. 不传入也不会有太大的作用域查询消耗.
(function() {
    ...
    */
    ...
    // 如果使用 _(obj) 的方式调用, 则需要做一些兼容处理,确保返回正确的 underscore 实例
    var _ = function(obj) {
        if (obj instanceof _)
            return obj;
    ...
    // 兼容 node 和浏览器两种. 之前有些版本并没有在 else 中处理 root._ = _;
    // 这会导致宿主和 require 两种方式都能获取 _
    ...
    var group = function(obj, value, context, behavior) {
        var result = {};
        var iterator = lookupIterator(value == null
            ? _.identity
            : value);
    ...
    _.initial = function(array, n, guard) {
        return slice.call(array, 0, array.length - ((n == null) || guard
            ? 1
            : n));
    };
